...
# icon = pygame.image.load("C:/Users/usimarit/Desktop/2025-2026-1/Introduction to AI/Assignment2/Pathfinder/Pathfinder/img/icon.png")
...
def draw_tile(x, y, tile_type):
  ...
  # 确保坐标在有效范围内
  if y < 20 or y >= HEIGHT or x < 0 or x >= WIDTH:
    return
  ...
  if node.type == 'wall' or node == start or node == end:
    return
...
def pathfind():
  if not start or not end:
    ...
    return
  # path, visitor_history = bfs(start, end)
  path, visitor_history = dfs(start, end)
  # path, visitor_history = a_star(start, end)
  update_visitors(display, grid, visitor_history)
  if not path:
