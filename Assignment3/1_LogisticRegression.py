...
功能：
1. 随机二分类数据集 + Logistic Regression + 嵌套交叉验证（10-fold 外层，5-fold 内层）
2. Adult 数据集上的 Logistic Regression 训练与评估：
   - 使用全部特征
   - 使用部分特征（子集）以比较性能差异
"""
...
def nested_cv_logistic_regression_random_data(
    n_samples=1000,
    n_features=20,
    n_informative=10,
    random_state=42
):
    print("===== Part 1: 随机二分类数据集上的 Logistic Regression 嵌套交叉验证 =====")
    ...
    for train_idx, test_idx in outer_cv.split(X, y):
        ...
        print(f"外层测试集: Accuracy={acc:.4f}, Precision={prec:.4f}, Recall={rec:.4f}, F1={f1:.4f}")
...
def logistic_regression_on_adult(
    train_path="adult.data",
    test_path="adult.test"
):
    print("\n===== Part 2: Adult 数据集上的 Logistic Regression 实验 =====")
...
if __name__ == "__main__":
    # Part 1: 随机数据集 + 嵌套交叉验证
    nested_cv_logistic_regression_random_data()

    # Part 2: Adult 数据集 + Logistic Regression
    # 需要确保当前目录下存在 adult.data 和 adult.test
    try:
        logistic_regression_on_adult("adult.data", "adult.test")
