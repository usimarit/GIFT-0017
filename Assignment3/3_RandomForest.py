...
功能：
1. 随机二分类数据集 + Random Forest + 嵌套交叉验证（10-fold 外层，5-fold 内层）
2. Adult 数据集上的 Random Forest 训练与评估：
   - 使用全部特征
   - 使用部分特征（子集）以比较性能差异
"""
...
def nested_cv_rf_random_data(
    n_samples=1000,
    n_features=20,
    n_informative=10,
    random_state=42
):
    print("===== Part 1: 随机二分类数据集上的 Random Forest 嵌套交叉验证 =====")
    ...
    # 对于随机数据，特征都是数值，直接用 RandomForest
    pipe = Pipeline(steps=[
        # Random Forest 本身对特征缩放不敏感，此处不加 scaler 也可以
...
def build_preprocessor(
    ...
    - 数值特征：StandardScaler（虽然对树模型影响不大，但保持风格统一）
...
if __name__ == "__main__":
    # Part 1: 随机数据集 + 嵌套交叉验证
    nested_cv_rf_random_data()

    # Part 2: Adult 数据集 + Random Forest
    # 需要确保当前目录下存在 adult.data 和 adult.test
    try:
        rf_on_adult("adult.data", "adult.test")
